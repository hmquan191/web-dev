...
const isAdmin = async (userId) => {
    ...
    const result = await query(sql, [userId]);
    return result[0] && result[0].role === 'admin';
};

// Modified deletePost function to check for admin role
...
// Modified deleteComment function to check for admin role
const deleteComment = async (commentId, userId) => {
    if (await isAdmin(userId)) {
        const sql = 'DELETE FROM comments WHERE id = ?';
        await query(sql, [commentId]);
    } else {
        throw new Error('Unauthorized: Only admins can delete comments.');
    }
};
